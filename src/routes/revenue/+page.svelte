<script>
	...
	$: totalRevData = d3
		.rollups(
			$filteredData
				.filter((d) => d['Revenue Tier'] !== 'NA')
				.filter((d) => d['Total Revenue'] !== ''),
			(v) => ({
	...
	$: totalExpensesData = d3
		.rollups(
			$filteredData
				.filter((d) => d['Expenses Tier'] !== 'NA')
				.filter((d) => d['Total Expenses'] !== ''),
			(v) => ({
...
<div class="charts__wrapper charts__revenue">
	<Chart
		...
		xDomain={[...new Set(revenueTierData.map((d) => d.group))].sort(
	...
	<Chart
		...
		flatData={flatten(
			d3.stack().keys(['Foundation', 'Other Charitable', 'Earned', 'Individual Giving'])(
				totalRevData
			)
		)}
		data={d3.stack().keys(['Foundation', 'Other Charitable', 'Earned', 'Individual Giving'])(
			totalRevData
		)}
		isWide={true}
	/>
	<Chart
		...
		flatData={flatten(
			d3
				.stack()
				.keys(['Rev. from Small Donors', 'Rev. from Mid-range Donors', 'Rev. from Major Donors'])(
				donorSizeDataRevenue
			)
		)}
		data={d3
			.stack()
	...
	<Chart
		...
		zRange={colorsCategorical}
		flatData={flatten(
			d3.stack().keys(['Small Donors', 'Mid-range Donors', 'Major Donors'])(donorSizeDataCount)
		)}
	...
	<Chart
		...
		zRange={colorsCategorical}
		flatData={flatten(
			d3.stack().keys(['Editorial', 'Revenue Generation', 'Technology', 'Administration'])(
				totalExpensesData
			)
		)}
		data={d3.stack().keys(['Editorial', 'Revenue Generation', 'Technology', 'Administration'])(
			totalExpensesData
		)}
		isWide={true}
	/>

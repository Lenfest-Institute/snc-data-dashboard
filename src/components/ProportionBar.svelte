<script>
	...
	$: stackedData = $data.reduce((acc, item) => {
		const cumulativeCount =
			acc.length > 0 ? acc[acc.length - 1].offset + acc[acc.length - 1].count : 0;
	...
	const colorScale = d3.scaleOrdinal().domain(zDomain).range(zRange); // Define `width` based on your chart size
</script>
